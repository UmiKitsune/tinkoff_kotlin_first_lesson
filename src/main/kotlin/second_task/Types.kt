...
fun determineType(obj: Any?) {
    ...
    val answer = when (obj) {
        is String -> "Я получил тип String = '$obj', ее длина равна ${obj.length} символ"
        ...
        //is Double -> "$obj -> ${floor(obj * 100.0) / 100.0}" //todo: вопрос - если после точки 1 число, нужно ли делать 2 через 0
        is LocalDate ->
            if (obj.isBefore(tinkoffFoundationDay))
                ...
                "Я получил LocalData = ${dateFormatter.format(obj)}, эта дата основания Tinkoff ฅ^•ﻌ•^ฅ"
            else
